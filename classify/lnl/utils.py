...
def make_data_loader(args, no_aug=False, transform=None, **kwargs):
    if args.dataset == 'cifar100':
        ...
        size1 = 256
        size = 224
    elif args.dataset in ['ds_custom', 'ds_custom2', 'ds_custom3']:
        #Replace by the values for your dataset
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        size1 = 224
    ...
    if args.dataset == "cifar100":
        ...
        from custom_dataset import make_ds_dataset, DualSteamCustom
        train_data, val_data, cname2id  = make_ds_dataset()
        trainset = DualSteamCustom(train_data, cname2id, transform=transform_train, transform_cont=transform_cont, cont=args.cont, consistency=True)
        trackset = DualSteamCustom(train_data, cname2id, transform=transform_test, transform_cont=transform_train, cont=False, consistency=False)
        testset = DualSteamCustom(val_data, cname2id, transform=transform_test)      
    else:
        raise NotImplementedError("Dataset {} is not implemented".format(args.dataset))
    

    train_loader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, **kwargs) #Normal training
    track_loader = DataLoader(trackset, batch_size=args.batch_size, shuffle=True, **kwargs)
